...
public class BroadcastReceiveThread extends Thread {
    ...
    public void run() {
        ...
        if (datagramSocket == null) {
            Log.e(TAG, "接收数据包线程 创建socket失败");
            return;
        }
        Log.d(TAG, "接收数据包线程 run: 开始运行");
        while (!isInterrupted()) {
            try {
            ...
            String gotJson = new String(gotData);
            Log.d(TAG, "接收数据包线程 run: address: " + datagramPacket.getAddress() + ", port: " + datagramPacket.getPort()
